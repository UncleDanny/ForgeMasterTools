(() => {
  ...
  const MAX_AS = 480;
  const STEP_AS = 0.001;
  const DEFAULT_ATTACK_SPEED = 151.000;
  const DEFAULT_DOUBLE_CHANCE = 51.200;
  ...
  async function init() {
    try {
      ...
      renderSkinOptions();
      $("skin").value = "0";
      $("attackSpeed").value = DEFAULT_ATTACK_SPEED.toFixed(3);
      $("doubleChance").value = DEFAULT_DOUBLE_CHANCE.toFixed(3);
