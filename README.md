...
Static GitHub Pages calculator for the game's 10-tick combat timing model.

## Data sources

The page now uses the supplied game data directly:
...
The timing lookup follows the same mapping used by the source project:
...
There is no manually maintained skin configuration file and no hardcoded Cat windup.
...
Publish the directory containing `index.html`, `app.js`, `styles.css`, `SkinsLibrary.json`, and `WeaponLibrary.json` as a static GitHub Pages site.

Both JSON files must remain beside `index.html` because the browser loads them with `fetch()` at runtime.
